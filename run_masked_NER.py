...
def run_trainer():
    ...
    helper += "             sci1: retrain from initialization using sciBERT  (b0, b1,   .. b4  );"
    helper += "             sci2: pretrain from previous checkpoint using sciBERT (b0, b+1,  .. b+4 );"
    helper += "             sci3: pretrain from previous checkpoint using sciBERT (b0, b+1_, .. b+4_);"
    helper += "             sci4: pretrain from previous checkpoint using sciBERT (b0, b1_,  .. b4_);"
    helper += "             bio[1-4]: same setting but using bioBERT;"
    helper += "    <rep1> <rep2> ..."
    helper += "           - tell which rep to run, must be integer {0, .., 4}."
    ...
    seed = [0, 2023, 604641129, 449958349, 196965608]
    map_running = {
    ...
    if mode_code in ['sci1', 'bio1']:
        ...
        for this_rep in rep:
            for ds in [base] + datasets: # for ds in datasets:
                for lr in grid_search:
                    cmd = f"bash maskedNER/{params[0]}.sh {ds} REP{this_rep+1} {lr} {seed[this_rep]} {params[1]} {params[2]} {which_bert}"
                    os.system(cmd)
    else:
        for this_rep in rep:
            if os.path.exists(f"outputs/REP{this_rep+1}/pmc60+_v0_g2_b0") and len(os.listdir(f"outputs/REP{this_rep+1}/pmc60+_v0_g2_b0/{which_bert}")) == 6:
