...
def run_predictor():
    ...
    OUT_BASE = f"outputs/{ACC}/{DS}/{BERT}BERT" # output/REP1/pmc60+_v0_g2_b0/sciBERT
    if not os.path.exists(OUT_BASE):
        print(f"Error: directory {OUT_BASE} doesn't exist, did you train the model?")
        exit()

    for subfdir in os.listdir(OUT_BASE):
        for ts in ['fullPaperTest']:
            outdir = f"{OUT_BASE}/{subfdir}" # output/REP1/pmc60+_v0_g2_b0/sciBERT/lrxxx
