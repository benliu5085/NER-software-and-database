...
def extract_flat_spans(start_pred, end_pred, match_pred, start_label_mask, end_label_mask, pseudo_tag = "TAG"):
    """
    ...
    start_label_mask = start_label_mask.bool()
    end_label_mask = end_label_mask.bool()
    start_pred = start_pred.bool()[0,:]
    end_pred = end_pred.bool()[0,:]
    match_pred = match_pred.bool()[0,:,:]

    bmes_labels = ["O"] * len(start_pred)
    start_positions = [idx for idx, tmp in enumerate(start_pred) if tmp and start_label_mask[0,idx]]
    end_positions = [idx for idx, tmp in enumerate(end_pred) if tmp and end_label_mask[0,idx]]
